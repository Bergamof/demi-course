...
- The turnaround marker on a step card also reports what is left of the **segment** the turnaround falls in — that one repetition's effort, or that one recovery phase, never the remaining repetitions of the step — expressed in distance or time depending on the `HalfBy` setting (`TurnPoint.restDistance` / `restTime`). The point is to be readable off a watch mid-run: 30 s into the first of 3 × 2 min, it says 1m30s, not 5m30s.
